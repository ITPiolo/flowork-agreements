...
export async function renderFilledPdf(html, fields, values, { signatureField, dateField, stampField, stampImageDataUrl, extraAnchorFields = [] } = {}) {
  ...
  try {
    ...
    const missing = await page.evaluate((specs, sig, dateAnchor, stamp, extraAnchors) => {
      ...
      // places DocuSign's Sign button on top of the label itself. Anchored inline, right after
      // the label text on the same row ("SIGNATURE: [sign here]") rather than on the blank
      // line below it — the caller pushes it further right with anchorXOffset to clear the
      // label's own text width.
      if (sigTarget) {
        const anchorSpan = document.createElement('span');
        anchorSpan.textContent = sig.id;
        anchorSpan.style.fontSize = '1px';
        anchorSpan.style.color = '#ffffff';
        sigTarget.parentNode.insertBefore(anchorSpan, sigTarget.nextSibling);
      }
      if (dateTarget) {

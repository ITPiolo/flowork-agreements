...
export async function renderFilledPdf(html, fields, values, { signatureField, dateField, stampField, stampImageDataUrl } = {}) {
  const isServerless = !!process.env.VERCEL;
  ...
  try {
    ...
    const dateSpec = dateField
      ...
      : null;
    const stampSpec = stampField && stampImageDataUrl
      ? { id: stampField.id, pattern: stampField.pattern, occurrence: stampField.occurrence, imageDataUrl: stampImageDataUrl }
      : null;

    const missing = await page.evaluate((specs, sig, dateAnchor, stamp) => {
      function looksBlank(el) {
        if (!(el instanceof Element)) return false;
        if (el.textContent.trim().length > 0) return false;
        const style = getComputedStyle(el);
      ...
      // Anchoring directly before the "SIGNATURE"/"DATE" label text (as text fields never do)
      // places DocuSign's Sign button and date stamp right on top of the label itself. Anchor
      // to the blank line below instead, same as every text field, falling back to the raw
      // label position only if no blank line is found nearby.
      if (sigTarget) {
      ...
      // needing to hand-measure the box.
      if (stampTarget && stamp.imageDataUrl) {
        ...
        // upload would otherwise render at its native pixel size, and width-only sizing could
        // distort a narrow/tall image once height also had to be capped. object-fit keeps the
        // image's own proportions intact within that fixed box instead of stretching it.
        img.style.width = '140px';
        img.style.height = '80px';
        img.style.marginTop = '10px';
        img.style.objectFit = 'contain';
        labelEl.insertAdjacentElement('afterend', img);
      }

      return missing;
    }, fieldSpecs, sigSpec, dateSpec, stampSpec);

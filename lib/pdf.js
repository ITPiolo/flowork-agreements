  ...
  try {
    ...
    const missing = await page.evaluate((specs, sig, dateAnchor, stamp, extraAnchors) => {
      ...
      for (const [blankEl, entries] of groups.entries()) {
        ...
        // The blank line is originally just a ~1px underline — barely any layout height of
        // its own. Growing it to fit the value's line-height (as earlier versions did) adds
        // that extra height on EVERY filled field; with 20-30 fields on a dense page, that
        // compounds into hundreds of pixels beyond what the page's fixed 297mm frame was
        // ever budgeted for (the template was authored assuming a blank, unfilled form),
        // pushing later content — including the page footer — past the frame, where
        // overflow:hidden silently deletes it. Positioning the value absolutely, floating in
        // the label's existing margin-gap just above the line, means filling in values never
        // changes the row's own layout height at all, no matter how many fields are filled.
        ...
        if (entries.length === 1) {
          ...
          // A value long enough to not fit on one line at readable size shrinks down instead
          // of wrapping, with an ellipsis as the last resort — since this element is now
          // absolutely positioned, it already shrink-wraps to its content width, so
          // scrollWidth genuinely reflects the rendered text width without needing any
          // particular display type.
          span.style.fontSize = '10px';
          let fontSize = 10;
          const available = blankRect.width;
          while (fontSize > 6 && span.scrollWidth > available) {
            fontSize -= 0.5;
            span.style.fontSize = fontSize + 'px';
          }
        } else {
          // Unlike single-value rows, this shared blank sits flush against its label (no
          // gap above) but has generous empty space reserved right below it — so the value
          // floats down into that space instead of up like the single-value case.

...
// House Rules signature block: "Signature:" / "Date:" / "Company name:" labels sit below a
// "Signature" section header — the header text alone also matches '^Signature', so the pattern
// must require the trailing colon to land on the actual label instead of the header above it.
export const HOUSE_RULES_FIELDS = [
  textField('house_rules_company_name', '^Company name:$', 'Company Name', { required: true }),
  textField('house_rules_date', '^Date:$', 'Date'),
...
// Not a DocuSign tab — the "COMPANY STAMP" box on the KYC signature page is filled with the
// client's own uploaded stamp image directly in renderFilledPdf, the same way text answers are
// written in as real page content, rather than something DocuSign itself needs to place.
export const KYC_STAMP_FIELD = { id: 'company_stamp', pattern: '^COMPANY STAMP$', occurrence: 0 };
